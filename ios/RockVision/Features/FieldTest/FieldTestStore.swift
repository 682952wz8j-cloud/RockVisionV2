...
final class FieldTestStore {
    ...
    func createSession(openCVVersion: String) throws -> FieldTestSessionHandle {
        ...
        try handle.writeSummary(FieldTestSummary(sessionID: id, updatedAt: Date(), phase: "idle", cells: []))
        return handle
    }

    func latestSession() -> FieldTestSessionHandle? {
        guard let dirs = try? fileManager.contentsOfDirectory(at: rootURL, includingPropertiesForKeys: [.creationDateKey], options: [.skipsHiddenFiles]) else {
            return nil
        }
