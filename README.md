...
| 5 | Route polylines in AR | **OPEN / IN PROGRESS** (opened 2026-08-27). Gate 5A **PASS / CLOSED**. First route **FROZEN** (`route_test_01`). Gate 5B **PASS / CLOSED**. Gate 5C **PASS / CLOSED**. Frozen route runtime load **PASS**. Frozen geometry hash gate **PASS**. Rolling runtime route binding **PASS**. Gate 5D **OPEN / IN PROGRESS / NOT PASS**. Gate 5D-A **PASS / CLOSED**. Route rendering (renderer correctness) **PASS**. Physical visual validation **NOT PERFORMED**. `GATE_5D_PASS = NO`. Gate 5D-B **NOT STARTED / NOT AUTHORIZED**. Route package **NOT CREATED / NOT AUTHORIZED**. Gate 5E **NOT STARTED**. Stage 5 **NOT PASS** |
...
Canonical ingested polyline: `validation/gate5a/gate5a_ingested_route_test_01.json` (geometry object; `gate5aPass` false; `FIRST_ROUTE_FROZEN` false). Gate 5A PASS closure: `validation/gate5a/gate5a_pass_closure_route_test_01.json`. Gate 5B transform correctness: `validation/gate5b/gate5b_route_transform_correctness_route_test_01.json`. Gate 5C runtime route binding: `validation/gate5c/gate5c_runtime_route_binding_route_test_01.json`. Gate 5D-A renderer correctness: `validation/gate5d/gate5da_renderer_correctness_route_test_01.json`. `GATE_5A_INPUT_READY = YES`. `DXF_TO_BLOCKR_RELATION = IDENTITY_PROVEN`. `WallMetricMeters = PROVEN`. `GATE_5A_PASS = YES`. `GATE_5B_PASS = YES`. `GATE_5C_PASS = YES`. `GATE_5D_A_PASS = YES`. `GATE_5D_PASS = NO`. That is **not** production Wall Package `routes.json` authorization and **not** physical visual validation / Gate 5D PASS.
...
**PASS / CLOSED**. Route
package creation is **NOT AUTHORIZED**. Gate 5C is **PASS / CLOSED**. Gate 5D is **OPEN / IN PROGRESS / NOT PASS**. Gate 5D-A is **PASS / CLOSED**. Route rendering (renderer correctness) is **PASS**. Physical visual validation is **NOT PERFORMED**. `GATE_5D_PASS = NO`. Gate 5D-B is **NOT STARTED / NOT AUTHORIZED**. Gate 5E is **NOT STARTED**. Stage 5 is
**NOT PASS**.
...
- Human review of Gate 5D-A PASS / CLOSED
- Gate 5D-B only after a later explicit protocol
...
Gate 5C is **PASS / CLOSED**. Gate 5D is **OPEN / IN PROGRESS / NOT PASS**.
Gate 5D-A is **PASS / CLOSED**. That is **not** production
Wall Package `routes.json` authorization, **not** physical visual
validation, and **not** Gate 5D PASS.

**Not allowed now**

- Creating `routes.json` / a production route package
- DXF in the app bundle
- Gate 5D-B physical visual validation without a later explicit protocol
- Declaring Gate 5D PASS or Stage 5 PASS
- Gate 5E without a later explicit protocol
...
- Tuning scale, offset, flip, or production `T` from marker or route appearance
