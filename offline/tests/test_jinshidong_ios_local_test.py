...
class JinshidongIOSLocalTestTests(unittest.TestCase):
    ...
    def test_stage5_debug_hud_is_localization_pnp_wall_and_cloud(self) -> None:
        ...
        self.assertIn("PnP \\(pnp.inliers)", hud)
        self.assertIn("medianToken", hud)
        self.assertIn("云端已加载", hud)
        self.assertIn("加载失败", hud)
        self.assertIn("ProductionRouteFieldCopy", hud)
        self.assertIn("🔗 \\(count)+2", hud)
        self.assertNotIn("wall_jinshidong_01", hud)
        self.assertNotIn("wall_jiulongfeng_01", hud)
        self.assertNotIn("云端资产：已加载", hud)
        self.assertNotIn("云端资产：加载失败", hud)
        ...
        self.assertIn("routes: openCV.productionFieldRoutes", content)
        ...
        self.assertIn("productionFieldRoutes", stage)
